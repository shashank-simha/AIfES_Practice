...
train_data = full_train.data[subset_indices].numpy()  # [10000,28,28] uint8
test_data = full_test.data.numpy()  # [10000,28,28] uint8

# Downsample to 14x14
train_down = np.mean(train_data.reshape(-1,14,2,14,2), axis=(2,4)).astype(np.float32)
test_down = np.mean(test_data.reshape(-1,14,2,14,2), axis=(2,4)).astype(np.float32)
...
class CustomMNIST(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = (img - MEAN) / STD
        img = torch.tensor(img).unsqueeze(0)  # [1,14,14]
        label = torch.tensor(self.targets[idx], dtype=torch.long)
        return img, label

train_dataset = CustomMNIST(train_down, full_train.targets[subset_indices].numpy())
test_dataset = CustomMNIST(test_down, full_test.targets.numpy())
...
class CNN(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 2, kernel_size=3, stride=1, padding=1)   # 1→2
        self.pool1 = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(2, 4, kernel_size=3, stride=1, padding=1)   # 2→4
        self.pool2 = nn.MaxPool2d(2, 2)
        self.fc1 = nn.Linear(4*3*3, 16)  # 36→16
